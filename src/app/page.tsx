...
export default async function HomePage() {
  const result = await serverFetch<Property[]>(
    "/properties/search?limit=6&sortBy=createdAt&sortOrder=desc",
    300,
  );
        ...
        <div className="container-page grid grid-cols-1 items-center gap-10 py-16 lg:grid-cols-2 lg:py-24">
          <div>
            ...
            <h1 className="font-display text-4xl leading-tight text-ink sm:text-5xl">
              Find a home,
              <br />
              drawn straight from the owner.
            </h1>
            <p className="mt-5 max-w-md text-ink/70">
              Every listing on Plinth connects you directly to the person who
              owns it — verified details, real photos, and a conversation with
              no one in between.
            </p>
            <div className="mt-8">
              <HomeSearchBar />
          ...
          <div className=" relative hidden aspect-[4/3] border border-line  lg:block">
            <div className="z-20 absolute inset-0 flex flex-col justify-between p-6 font-mono text-xs text-brass font-semibold">
              <div className="flex justify-between">
                <span>SITE PLAN</span>
                <span>SCALE 1:100</span>
              </div>

              <div className="grid grid-cols-3 gap-4 text-brass">
          ...
          {[
            {
              ...
              body: "Filter by city, budget, and bedrooms with results that stay fast even at scale.",
            },
            {
              icon: ShieldCheck,
              step: "02",
              title: "Message the owner",
              body: "Send one inquiry per listing — owners see it immediately, no forwarding through an agent.",
            },
            {
              icon: Wallet,
              step: "03",
              title: "Move in, brokerage-free",
              body: "Agree terms directly. No commission split, no hidden listing fee.",
            },
          ].map(({ icon: Icon, step, title, body }) => (
            <div key={step}>
              <div className="mb-3 flex items-center gap-2 font-mono text-xs text-ink/40">
                {step}
              </div>
